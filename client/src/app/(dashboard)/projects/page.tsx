...
export default function ProjectsPage() {
  ...
  const loadProjects = async () => {
    try {
      const { ProjectsApi, UsersApi } = await import('@/lib/api');
      ...
      const mapped = filteredData.map((p: any) => ({
        id: p.id,
        title: p.name,
        client: {
  ...
  return (
    <div className="h-full flex flex-col font-sans animate-in fade-in slide-in-from-bottom-4 duration-500 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto w-full">
